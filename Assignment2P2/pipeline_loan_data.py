...
class start_task(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget('Logs/pipeline_loan_data.log')

    def run(self):
        logFilePath = 'Logs/pipeline_loan_data.log'
        funCheckDir(logFilePath)
        logger = getLogger(logFilePath)
        logger.info("Pipeline for loan-data started....")
        ...
        # filePath = 'CleanedData/LoanData.csv'
        filePath = 'CleanedData/LoanData.csv'
        ...
        #Logging finished
        logger.info("Pipeline for loan-data finished....")
        logging.shutdown()

if __name__ == '__main__':
    try:
        os.remove('Logs/pipeline_loan_data.log')
    except OSError:
        pass
    luigi.run()
