...
class start_task(luigi.Task):
    ...
    def requires(self):
        return Clean_declined_loan_data()

    def output(self):
        return luigi.LocalTarget('Logs/pipeline_declined_loan_data.log')

    def run(self):
        logFilePath = 'Logs/pipeline_declined_loan_data.log'
        funCheckDir(logFilePath)
        logger = getLogger(logFilePath)
        logger.info("Pipeline for declined-loan-data started....")
        ...
        filePath = 'CleanedData/LoanDeclinedData.csv'
        ...
        #Logging finished
        logger.info("Pipeline for declined-loan-data finished....")
        logging.shutdown()

if __name__ == '__main__':
    try:
        os.remove('Logs/pipeline_declined_loan_data.log')
    except OSError:
        pass
    luigi.run()
